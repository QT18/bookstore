...
class Database
{
    ...
    {
        $this->link = mysqli_connect("localhost","root","","luanvan") or die();
    ...
    public function insert($table, array $data)
    {
        ...
        $sql .= " VALUES (" . $values . ')';
        // _debug($sql);die;
        mysqli_query($this->link, $sql) or die("Lỗi truy vấn chèn dữ liệu ----" .mysqli_error($this->link));
    ...
    public function update($table, array $data, array $conditions)
    {
        ...
        $sql .= $set . $where;
        // _debug($sql);die;
        mysqli_query($this->link, $sql) or die("Lỗi truy vấn cập nhật ----" .mysqli_error($this->link));
        return mysqli_affected_rows($this->link);
    }

    public function updateview($sql)
    {
        $result = mysqli_query($this->link,$sql) or die("Lỗi cập nhật hiển thị ----" .mysqli_error($this->link));
        return mysqli_affected_rows($this->link);
    }
    ...
    public function deletewhere($table, $data = array())
    {
        foreach ($data as $id)
        {
            $id = intval($id);
            $sql = "DELETE FROM {$table} WHERE id = $id";
            mysqli_query($this->link,$sql) or die("Lỗi truy vấn deletewhere ----" .mysqli_error($this->link));
        }
        return true;
    }
